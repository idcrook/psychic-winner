...
// Description:
//
//   Holders for bottle cage for sleeve mount system family
//
// Revisions/Notes:
//
...
screw_hole_spacing = 60.0;
cage_bottom_platform_from_bottom_screw_hole = 40;
sleeve_placement_height = cage_bottom_platform_from_bottom_screw_hole + 0;
sleeve_insert_from_bottom_screw_hole = 20;


screw_size = 5.0; // M5
hex_nut_thickness = 3.0;
hex_nut_width = 9.0;
...
if (RENDER_FOR_PRINT) {
  //test_sleeveMountInsert(true, 0);
 } else {
  translate ([0,0,50])
    rotate ([90, 0, 0])
    import ("bottle_holder_bar.stl");

  translate ([0,0,50])
  test_sleeveMountInsert(true, 0);
     xdistribute(spacing=25){
        nut("M5",thickness="normal");
        nut("M5",thread="none");
        screw("M5", length=12, anchor=BOT, head="button", drive="hex");
